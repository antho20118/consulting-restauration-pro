...
// Extrait le nom, les portions, les ingrédients (avec quantité/unité) et les étapes d'un texte de
// recette collé librement par l'utilisateur. Ne fait aucun rapprochement avec les articles de la
// base (ambigu et coûteux à faire faire par le modèle sur un catalogue de plusieurs milliers de
// références) : ce rapprochement est fait ensuite côté client, qui a déjà la liste des articles
...
export async function extraireRecetteDepuisTexte(
  ...
  const reponse = await client.messages.parse({
    ...
    system:
      "Tu extrais les informations structurées d'une recette de cuisine collée par l'utilisateur, " +
      "en français. Pour chaque ingrédient, choisis l'unité la plus proche parmi celles fournies dans " +
      "le schéma ; si aucune ne correspond vraiment, mets null plutôt que d'en inventer une. Le champ " +
      "nomExtrait ne doit contenir que le nom de l'aliment, sans quantité ni unité.",
