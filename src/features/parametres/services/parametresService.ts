...
export async function modifierCategorieRecette(id: number, nom: string): Promise<CategorieRecette> {
  const response = await apiFetch(`${API_URL}/categories-recette/${id}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ nom }),
  });
...
export async function modifierUnite(id: number, input: UniteInput): Promise<Unite> {
  const response = await apiFetch(`${API_URL}/unites/${id}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(input),
  });
...
export async function getTva(): Promise<Tva[]> {
  const response = await apiFetch(`${API_URL}/tva`);
...
export async function modifierTva(id: number, input: TvaInput): Promise<Tva> {
  const response = await apiFetch(`${API_URL}/tva/${id}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(input),
  });
...
export async function supprimerTva(id: number): Promise<void> {
  const response = await apiFetch(`${API_URL}/tva/${id}`, { method: "DELETE" });
